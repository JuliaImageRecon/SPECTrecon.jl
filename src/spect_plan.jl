...
struct SPECTplan
    ...
    function SPECTplan(mumap::AbstractArray{<:RealU, 3},
        ...
        new(T, imgr, mumap, mumapr, psfs, nview, viewangle, interpidx, dy,
            imgsize, psfsize, pad_fft, pad_rot, ncore)
...
# 82.389 ms (199856 allocations: 97.83 MiB)
# @btime workarray = Vector{Workarray}(undef, plan.ncore)
# 187.780 ns (2 allocations: 1.17 KiB)
# @btime for i = 1:plan.ncore
#     workarray[i] = Workarray(plan.T, plan.imgsize, plan.pad_fft, plan.pad_rot) # allocate
# end
# 3.804 ms (3962 allocations: 7.59 MiB)
